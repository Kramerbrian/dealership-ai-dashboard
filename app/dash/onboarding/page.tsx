      ...
      <div className="w-full max-w-xl rounded-3xl border border-white/10 bg-white/[0.04] p-6 md:p-8">
          ...
          <section className="mt-5 space-y-3">
            ...
            <div className="flex gap-3">
              <input
                ...
                className="w-1/2 h-10 rounded-full bg-white/5 border border-white/15 px-4 text-sm textWhite placeholder:text-white/30 focus:outline.none focus:ring-2 focus:ring-white/40"
              />
              <input
                placeholder="State"
                value={state}
        ...
        {step === 2 && (
          <section className="mt-5 space-y-3">
            ...
            <input
              ...
              className="mt-3 w-full h-10 rounded-full bg.white/5 border border-white/15 px-4 text-sm textWhite placeholder:text-white/30 focus:outline.none focus:ring-2 focus:ring-white/40"
            />
            <input
              placeholder="Average front-end gross"
              value={avgGross}
              onChange={(e) => setAvgGross(e.target.value)}
              className="w-full h-10 rounded-full bg.white/5 border border-white/15 px-4 text-sm textWhite placeholder:text-white/30 focus:outline.none focus:ring-2 focus:ring-white/40"
            />
          </section>
        )}
        ...
        {step === 3 && (
          <section className="mt-5 space-y-3">
            ...
            <p className="text-sm textWhite/60">
              We will adjust views and language based on your role.
            </p>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="mt-3 w-full h-10 rounded-full bg.white/5 border border-white/15 px-4 text-sm textWhite focus:outline.none focus:ring-2 focus:ring-white/40"
            >
        ...
        <div className="mt-6 flex items-center justify-between">
          <button
            ...
            onClick={back}
            disabled={step === 0}
            className="h-9 px-4 rounded-full border borderWhite/20 text-xs textWhite/70 disabled:opacity-40"
          >
            Back
          </button>
          <button
            type="button"
            onClick={next}
            className="h-9 px-5 rounded-full bg.white textBlack text-xs fontMedium hover:bgNeutral-100 transition"
          >
            {step === steps.length - 1 ? 'Finish' : 'Next'}
          </button>
        </div>

        <p className="mt-4 text-[11px] textWhite/40">
