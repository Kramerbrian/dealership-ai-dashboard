...
export default function VerificationBadge({ verified, tenant, domain }:{ verified:boolean; tenant?:string; domain?:string; }){
  ...
  return (
    <div className="flex items-center gap-2">
      {ok ? (
        <span className="inline-flex items-center gap-1 text-green-400 text-sm"><ShieldCheck className="w-4 h-4"/> Verified</span>
      ) : (
        <span className="inline-flex items-center gap-1 text-yellow-400 text-sm"><ShieldAlert className="w-4 h-4"/> Pending</span>
      )}
