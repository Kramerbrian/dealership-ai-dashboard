/**
 * Cursor Deeplink Generator for DealershipAI
 * Generates deeplinks for both desktop and web versions of Cursor
 */

export function generateDesktopPromptDeeplink(promptText: string): string {
...
export interface DealershipAIPrompt {
  ...
  description: string;
  prompt: string;
  category: 'api' | 'ui' | 'auth' | 'monitoring' | 'analytics' | 'integration' | 'documentation';
  complexity: 'beginner' | 'intermediate' | 'advanced';
}

export const DEALERSHIP_AI_PROMPTS: DealershipAIPrompt[] = [
  {
...
export function generatePromptDeeplinks(promptName: string, useWeb: boolean = false): string {
...
export function generateAllPromptDeeplinks(useWeb: boolean = false): Record<string, string> {
...
export function getPromptsByCategory(category: DealershipAIPrompt['category']): DealershipAIPrompt[] {
  return DEALERSHIP_AI_PROMPTS.filter(prompt => prompt.category === category);
}

export function getPromptsByComplexity(complexity: DealershipAIPrompt['complexity']): DealershipAIPrompt[] {
  return DEALERSHIP_AI_PROMPTS.filter(prompt => prompt.complexity === complexity);
}

export function printAllPrompts(useWeb: boolean = false): void {
  console.log("🚀 DealershipAI Cursor Deeplink Generator");
  console.log("=" * 50);
  
  const categories = ['api', 'ui', 'auth', 'monitoring', 'analytics', 'integration', 'documentation'] as const;
