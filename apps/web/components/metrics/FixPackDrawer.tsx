...
type Estimate = {
  ok: boolean;
  estimatedRecovery: number; // monthly $
  confidence: number; // 0..1
  breakdown: { kind: FixKind; lift: number; notes: string }[];
};
  ...
  return (
    <div className="fixed inset-0 z-[96]">
      ...
      <motion.aside
        ...
        className="absolute right-0 top-0 w-[440px] h-full bg-white/05 backdrop-blur-xl border-l border-white/10 text-white"
      >
        ...
        <div className="p-4 space-y-6 overflow-y-auto h-[calc(100%-64px)]">
          {/* Fix selection */}
          <section className="p-4 rounded-xl bg-white/08 border border-white/10">
            <div className="text-sm font-semibold mb-3">Choose fixes</div>
            <div className="grid grid-cols-2 gap-2">
              {(["schema","review","cwv","nap"] as FixKind[]).map((k) => (
                <label key={k} className="flex items-center gap-2 p-2 rounded bg-white/06 hover:bg-white/12 cursor-pointer">
                  <input
                    type="checkbox"
            ...
            <div className="mt-3">
              <button
                ...
                className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-sky-600 hover:bg-sky-500 text-sm disabled:opacity-40"
              >
                ...
                {estimate.breakdown.map((b, i) => (
                  <div key={i} className="p-3 rounded bg-white/06 border border-white/10">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-sm text-emerald-300">+${b.lift.toLocaleString()}</div>
                    </div>
          ...
          <section className="p-4 rounded-xl bg-white/08 border border-white/10">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40"
            >
