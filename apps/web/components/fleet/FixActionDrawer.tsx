    ...
    <div className='fixed inset-0 z-50 bg-black/50'>
      <div className='absolute right-0 top-0 h-full w-[600px] bg-neutral-900 border-l border-white/10 p-5 overflow-y-auto'>
        <div className='flex items-center justify-between mb-4'>
          <div className='text-lg font-semibold'>Fix now — {origin}</div>
          <button onClick={onClose} className='px-3 py-1 rounded-md border border-white/15'>Close</button>
        </div>
        ...
        <div className='space-y-3'>
          ...
          <div className='flex gap-2'>
            {(['autodealer','faq','vehicle'] as const).map(k=>(
              <button key={k} onClick={()=> setMode(k)}
                className={`px-3 py-1 rounded ${mode===k?'bg-primary-600 text-white':'bg-neutral-800 border border-white/10'}`}>
                {k.toUpperCase()}
              </button>
            ))}
          </div>
          ...
          <div className='grid grid-cols-2 gap-3'>
            <div>
              ...
              <pre className='h-48 overflow-auto bg-neutral-800 p-2 rounded border border-white/10 text-xs'>{baseline}</pre>
            </div>
            <div>
              <div className='text-xs text-neutral-400 mb-1'>Proposed JSON-LD</div>
              <textarea className='w-full h-48 bg-neutral-800 border border-white/10 rounded p-2 font-mono text-xs'
          ...
          <button className='btn disabled:opacity-40' disabled={saving} onClick={submit}>
...
function RollbackPanel({ origin }:{ origin:string }){
  ...
  return (
    <div className='mt-6'>
      <div className='text-sm font-semibold mb-2'>Rollback</div>
      <div className='space-y-2'>
        ...
        {versions.map(v=>(
          <div key={v.version_id} className='flex items-center justify-between border border-white/10 rounded p-2'>
            ...
            <button className='px-3 py-1 rounded-md border border-white/15 hover:bg-white/10 disabled:opacity-40'
              disabled={loading} onClick={()=> rollback(v.version_id)}>
              Rollback
            </button>
          </div>
        ))}
