...
import { Lock, Clock, ArrowUpRight, Sparkles } from 'lucide-react';
 ...
 * 
 * Protects Zero-Click drawer feature with tier-based access:
 * - Tier ≥ 2 → unlocked
 * - Tier 1 + active trial cookie → unlocked
 * - Else → gray overlay with "Enable 24h Trial" and Upgrade buttons
 */
export function ZeroClickDrawerGuard({ tier, children }: DrawerGuardProps) {
  const [isTrialActive, setIsTrialActive] = useState(false);
  const [isGranting, setIsGranting] = useState(false);
  const { data: trialStatus, mutate: checkTrial } = useTrialFeature('zero_click_drawer');
  ...
  const handleEnableTrial = async () => {
    ...
    try {
      const response = await fetch('/api/trial/grant', {
        ...
        body: JSON.stringify({ feature_id: 'zero_click_drawer' }),
      });
        ...
        <div className="flex flex-col items-center justify-center h-full p-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-sm">
            <button
              ...
              disabled={isGranting}
              className="flex-1 inline-flex items-center justify-center space-x-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg font-medium transition-colors"
            >
              {isGranting ? (
                <>
