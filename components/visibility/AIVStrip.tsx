...
export default function AIVStrip({
  ...
  const thr: Thresholds = useMemo(() => {
    return (["ChatGPT", "Perplexity", "Gemini", "Copilot"] as EngineName[])
      .reduce((acc, e) => ({
        ...acc,
        [e]: {
  ...
  if (!engines.length) {
    return <div className={`text-white/60 text-sm ${className || ""}`}>No engines enabled</div>;
  }
      ...
      {engines.map((e) => {
        ...
        return (
          <div key={e.name} className="flex items-center gap-2">
            <span className={`inline-block h-2 w-2 rounded-full ${dotColor(level)}`} />
            <span className="text-sm text-white/80">{e.name}</span>
            <span className={`text-sm ${textColor(level)} font-medium`}>{e.presencePct}%</span>
          </div>
        );
      })}
      <span className="text-xs text-white/40">
        checked {data?.lastCheckedISO ? timeAgo(new Date(data.lastCheckedISO)) : "now"}
      </span>
    </div>
  );
}
