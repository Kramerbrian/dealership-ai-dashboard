...
function useDocs() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setData({
        overview: 'Error loading documentation',
        sequenceMermaid: 'graph TD\n  A[Error] --> B[Unable to load]',
        exampleSdkJson: { error: true },
      });
    } finally {
      setLoading(false);
    }
            ...
            <div className="flex gap-2">
              <Copy
                ...
                getText={() => {
                  ...
                  if (activeTab === 'mermaid') return data.sequenceMermaid;
                  if (activeTab === 'json') return JSON.stringify(data.exampleSdkJson, null, 2);
                  return '';
                }}
              />
              <Copy
