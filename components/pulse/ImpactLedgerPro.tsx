...
export default function ImpactLedgerPro({
  ...
  const total = useMemo(() => {
    return receipts.reduce(
      (s, r) => s + (typeof r.deltaUSD === "number" ? r.deltaUSD : 0),
      0
    );
  }, [receipts]);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="text-sm px-3 py-1.5 rounded-full border border-white/15"
          >
            Export CSV
          </button>
          <button
              ...
              {r.undone ? (
                ...
                <span
                  className={`font-medium ${
                    r.deltaUSD >= 0 ? "text-green-400" : "text-red-400"
                  }`}
                >
