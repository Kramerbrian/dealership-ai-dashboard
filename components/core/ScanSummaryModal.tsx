  ...
  return (
    <div className="fixed inset-0 z-[98]">
      ...
      <motion.div
        ...
        exit={{ opacity: 0, y: 24 }}
        className="absolute left-1/2 -translate-x-1/2 top-16 w-full max-w-2xl bg-white/05 backdrop-blur-xl rounded-2xl border border-white/10 text-white overflow-hidden shadow-2xl"
      >
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-xl border border-white/10 bg-white/08">
          ...
          <div className="p-4 rounded-xl bg-red-900/20 border border-red-500/40 flex items-center justify-between">
            <div>
              ...
              <div className="text-3xl font-light text-red-300">
                ${((summary.rar_monthly || 0) / 1000).toFixed(1)}K
              </div>
            </div>
            <button
              onClick={onOpenRAR}
...
function ActionButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="w-full px-4 py-3 rounded-lg border border-white/10 bg-white/06 hover:bg-white/12 text-left flex items-center justify-between"
    >
