...
const DealershipAIDashboardLA: React.FC = () => {
  ...
  const deploySiteInject = async (name: string) => {
    ...
    try {
      ...
      const data = await res.json();
      openModal({ title: `Deployment Result`, body: <p>{data.message || "Deployed schema successfully"}</p> });
    } catch (err) {
      openModal({ title: "Deployment Error", body: <p>Failed to deploy schema.</p> });
    } finally {
      setLoading(false);
    }
  };
  ...
  const deployAutoImplement = async (factor: string) => {
    if (!user) return;
    openModal({ title: `Auto‑Implement ${factor}`, body: <p>Starting bulk deployment…</p> });
  ...
  const refreshMetrics = async () => {
    ...
    try {
      ...
      const data = await res.json();
      openModal({ title: "Refresh Started", body: <p>{data.message || "Your data refresh has started."}</p> });
    } catch (err) {
      openModal({ title: "Refresh Error", body: <p>Unable to start refresh.</p> });
    } finally {
      setLoading(false);
    }
  };
