...
export async function schemaToPulses(domainOrUrl?: string): Promise<Pulse[]> {
  ...
  try {
    ...
    const qs = domainOrUrl?.startsWith("http") ? `url=${encodeURIComponent(domainOrUrl)}` : `domain=${encodeURIComponent(domainOrUrl || "")}`;

    const res = await fetch(`${baseUrl}/api/schema/validate?${qs}`, { cache: "no-store" });
    ...
    if (missingProduct || missingDealer) {
      ...
      pulses.push({
        ...
        prescription:
          ...
          "Inject JSON-LD Product on VDPs and AutoDealer on homepage; revalidate via Rich Results.",
    ...
    if (missingFAQ) {
      ...
      pulses.push({
        ...
        prescription:
          ...
          "Generate 6–10 Q&As; embed FAQPage JSON-LD on /service and index pages.",
    ...
    if (invalidJSONLD && pulses.length === 0) {
      ...
      pulses.push({
        ...
        prescription:
          ...
          "Validate with Google's Rich Results; fix braces/quotes; re-deploy.",
