...
export async function ga4ToPulses(domain?: string): Promise<Pulse[]> {
  try {
    const res = await fetch(`/api/ga4/summary?domain=${encodeURIComponent(domain || '')}`, {cache: 'no-store'});
    if (!res.ok) return [];
    ...
    if (needsAEO) {
      pulses.push({
        ...
        prescription: 'Add FAQ schema to top service pages and publish entity-rich answers. Target +5–8% AI-assisted share.',
    ...
    if (g.bounceRatePct > 55) {
      pulses.push({
        ...
        prescription: 'Compress hero images, defer non-critical scripts, and surface primary CTA higher on page.',
