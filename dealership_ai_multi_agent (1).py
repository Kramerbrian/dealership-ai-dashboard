...
class AutoResponseAgent:
    """Generate witty, sarcastic auto‑responses for reviews or messages.

    This agent is inspired by Ryan Reynolds' quick‑witted humour and
    Dave Chappelle's impeccable timing. It crafts playful replies to
    customer reviews that strike a balance between charm, sarcasm and
    professionalism. The goal is to make responses memorable without
    appearing unprofessional. References to pop culture (like a certain
    film’s ludicrous mode) are woven in subtly; only the well‑versed
    will catch them.
    """

    def __init__(self) -> None:
        # Predefined templates. In a production setting you might
        # generate these dynamically via an LLM. Here we use a static
        # collection for determinism.
        self.templates: List[str] = [
            "Hey there! Thanks for the feedback. We promise our service is faster than a spaceship in *that* parody movie—ludicrously fast, you might say.",
            "Appreciate your honesty! We'll work on our delivery speed. Next time, we'll go beyond plaid.",
            "Thanks for stopping by. We strive to keep things smoother than a superhero’s banter. Your next visit will be Deadpool‑worthy.",
            "We hear you. Our team is taking this to heart, and like any good comedian, our timing will be impeccable next time.",
            "Thanks for the review! We'll make sure our next performance hits the punchline just right—no mic drop needed.",
        ]

    def generate(self, business_name: str, review_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a set of witty auto‑responses based on review metrics.

        Args:
            business_name: The name of the dealership.
            review_data: Dictionary containing ratings, counts and response rates.

        Returns:
            A dictionary mapping review sources to suggested responses. Only
            sources with response rates below 0.5 will receive a
            suggestion.
        """
        import random
        suggestions: Dict[str, str] = {}
        for source, rate in review_data.get("response_rates", {}).items():
            # Only propose responses for platforms where the response
            # rate is low, indicating the team needs to reply.
            if rate < 0.5:
                template = random.choice(self.templates)
                # Personalise the template with the dealership name. We
                # avoid obvious puns but add subtle allusions. The
                # replacement is minimal to preserve wit.
                response = template.replace("our", f"{business_name}'s")
                suggestions[source] = response
        return {"suggestions": suggestions, "note": "Responses are tongue‑in‑cheek; adjust tone as needed."}
...
class IntegrationAgent:
    ...
    def __init__(self, platform_agent: PlatformAgent, analysis_agent: AnalysisAgent,
                 competitor_agent: CompetitorAgent, review_agent: ReviewAgent,
                 auto_response_agent: AutoResponseAgent | None = None) -> None:
        self.platform_agent = platform_agent
        self.analysis_agent = analysis_agent
        self.competitor_agent = competitor_agent
        self.review_agent = review_agent
        # Optionally accept an auto response agent. When supplied, the
        # integration will use it to craft witty responses to reviews
        # with low response rates.
        self.auto_response_agent = auto_response_agent
    ...
    async def run(self, business_name: str, location: str) -> Dict[str, Any]:
        ...
        # Optionally generate witty auto responses if an agent was provided.
        auto_responses: Optional[Dict[str, Any]] = None
        if self.auto_response_agent is not None:
            auto_responses = self.auto_response_agent.generate(business_name, review_data)
        # Combine all reports
        report: Dict[str, Any] = {
        ...
        if auto_responses is not None:
            report["auto_responses"] = auto_responses
        return report
