...
the dummy data with real API calls—without affecting the rest of
the system.
...
class PlatformAgent:
    ...
    async def run(self, query: str) -> List[SearchResult]:
        ...
        for result in results:
            if isinstance(result, SearchResult):
                ...
                # If an exception occurred, log it and mark the platform as unavailable
    ...
    async def _search_gemini(self, query: str) -> SearchResult:
        """Search Google's Gemini API for the query."""
        platform = "Gemini"
        ...
        body = {
            "prompt": {"text": f"Search for the following query and determine if the dealership appears: {query}"},
            "model": "gemini-pro",
            "temperature": 0.0,
        }
...
class AnalysisAgent:
    ...
    def __init__(self) -> None:
        self.avg_deal_value = 35000.0
        self.conversion_rate = 0.03
        self.search_volume = 1000
