...
# This script applies all Pulse-related migrations to ensure database is ready
...
for migration in "${PULSE_MIGRATIONS[@]}"; do
  ...
  if [ "$USE_LOCAL" = true ]; then
    # Apply to local
    if supabase db execute -f "$migration" 2>/dev/null; then
      echo -e "${GREEN}✅ Applied successfully${NC}\n"
    else
      echo -e "${YELLOW}⚠️  Migration may have already been applied or contains errors${NC}\n"
    fi
  else
...
echo -e "  1. Verify schema: ${GREEN}./scripts/verify-pulse-database.sh${NC}"
