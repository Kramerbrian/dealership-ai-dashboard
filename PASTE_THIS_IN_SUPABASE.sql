...
CREATE POLICY IF NOT EXISTS ati_tenant_sel ON ati_signals FOR SELECT USING (tenant_id = current_setting('app.tenant')::uuid);

CREATE POLICY IF NOT EXISTS ati_tenant_ins ON ati_signals FOR INSERT WITH CHECK (tenant_id = current_setting('app.tenant')::uuid);

CREATE POLICY IF NOT EXISTS ati_tenant_upd ON ati_signals FOR UPDATE USING (tenant_id = current_setting('app.tenant')::uuid) WITH CHECK (tenant_id = current_setting('app.tenant')::uuid);
